...
void PokerGame::gameInit() {
    ...
    for (int i = 0; i < numOfPlayers; ++i) {
        ...
        cin >> name;
        playerHands.emplace_back(deck.dealHand(gameType == 1 ? 5 : 2));
    ...
    for (int i = 0; i < turns; ++i) {
        ...
        for (int j = 0; j < numOfPlayers - 1; ++j) {
            ...
            for (int k = j + 1; k < numOfPlayers; ++k) {
                if (playerHands[j] > playerHands[k]) hands[k] = true;
        ...
        for (int j = 0; j < numOfPlayers; ++j) {
            playerHands.emplace_back(deck.dealHand(5));
...
void PokerGame::setGameType(unsigned short gameType_) {
    (gameType_ > 0 && gameType_ < 3) ? this->gameType = gameType_ : this->gameType = 1;
}
